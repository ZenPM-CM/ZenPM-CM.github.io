let isFetchingVideos = false; // Variable para evitar múltiples llamadas

export async function fetchYouTubeVideos(searchTerm = '', sortBy = 'date') {
    if (isFetchingVideos) return; // Evitar múltiples llamadas simultáneas
    ...
    try {
        ...
        if (videoGrid) {
            // Clear the video grid to prevent duplicates
            videoGrid.innerHTML = '';
        ...
        isFetchingVideos = false; // Reset the flag
    }
}
